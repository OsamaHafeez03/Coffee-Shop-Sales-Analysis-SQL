...
alter table sales
rename column ï»¿transaction_id to transaction;
...
SELECT 
    MONTH(transaction_date) AS month,  -- num of month --
    ...
    OVER (ORDER BY MONTH(transaction_date))) / LAG(SUM(unit_price * transaction_qty), 1)  -- division by sales
...
-- TOTAL ORDERS
SELECT COUNT(transaction) as Total_Orders
FROM sales 
...
SELECT 
    ...
    ROUND(COUNT(transaction)) AS total_orders,
    (COUNT(transaction) - LAG(COUNT(transaction), 1) 
    OVER (ORDER BY MONTH(transaction_date))) / LAG(COUNT(transaction), 1) 
...
SELECT
    ...
    round(count(transaction), 1) AS total_orders
FROM 
    sales
WHERE 
...
SELECT 
    ...
    CONCAT(ROUND(COUNT(transaction) / 1000, 1),'K') AS total_orders,
